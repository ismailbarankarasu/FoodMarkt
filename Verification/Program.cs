...
try
{
    ...
    foreach (var path in new[] { "/", "/Product", "/Product/Search?search=Test", "/Product/Category/" + category.Id, "/Product/Detail/" + product.Id })
    {
        ...
        Check(!page.Contains("href=\"#\"") && !page.Contains("Favorilere Ekle") && !page.Contains("Lorem ipsum") && !page.Contains("Free HTML Template"), "No dead controls or theme text: " + path);
        Check(Regex.IsMatch(page, "<img[^>]*src=\"/FoodMart-1.0.0/images/logo.png\"[^>]*alt=\"FoodMart ana sayfa\""), "Original header logo preserved: " + path);
        Check(page.Contains("action=\"/Cart/Add\"") && page.Contains("Sepete Ekle"), "Working add-to-cart form: " + path);
    ...
    Check(WebUtility.HtmlDecode(await Get("/Cart")).Contains("Sepetiniz henüz boş"), "New visitor has empty cart");
    Check((await http.PostAsync("/Cart/Add", new FormUrlEncodedContent(new Dictionary<string,string> { ["id"] = product.Id, ["quantity"] = "1" }))).StatusCode == HttpStatusCode.BadRequest, "Cart requires antiforgery token");
    Check((await http.GetAsync("/Cart/Add?id=" + product.Id)).StatusCode is HttpStatusCode.NotFound or HttpStatusCode.MethodNotAllowed, "GET cannot mutate cart");
    var added = await Post("/Cart/Add", new() { ["id"] = product.Id, ["quantity"] = "2", ["price"] = "0.01", ["returnUrl"] = "/Product" }, "/");
    Check(added.StatusCode == HttpStatusCode.Redirect && added.Headers.Location!.ToString() == "/Product", "Add product and return to listing");
    var cartPage = WebUtility.HtmlDecode(await Get("/Cart"));
    Check(cartPage.Contains("2.000,50") && cartPage.Contains("2 adet ürün"), "Cart uses current server discount price and persists quantity");
    Check(WebUtility.HtmlDecode(await Get("/")).Contains("Sepetim, 2 ürün"), "Header displays live cart quantity");
    using (var separate = new HttpClient(new HttpClientHandler { CookieContainer = new CookieContainer() }) { BaseAddress = http.BaseAddress })
        Check(WebUtility.HtmlDecode(await separate.GetStringAsync("/Cart")).Contains("Sepetiniz henüz boş"), "Visitor carts are isolated");
    await Post("/Cart/Add", new() { ["id"] = product.Id, ["quantity"] = "1" }, "/");
    Check(WebUtility.HtmlDecode(await Get("/Cart")).Contains("3 adet ürün"), "Adding same product increases quantity");
    await Post("/Cart/Update", new() { ["id"] = product.Id, ["quantity"] = "4" }, "/Cart");
    Check(WebUtility.HtmlDecode(await Get("/Cart")).Contains("4 adet ürün"), "Cart quantity update");
    await Post("/Cart/Update", new() { ["id"] = product.Id, ["quantity"] = "6" }, "/Cart");
    Check(WebUtility.HtmlDecode(await Get("/Cart")).Contains("4 adet ürün"), "Stock overflow rejected without changing quantity");
    await Post("/Cart/Update", new() { ["id"] = product.Id, ["quantity"] = "-1" }, "/Cart");
    Check(WebUtility.HtmlDecode(await Get("/Cart")).Contains("4 adet ürün"), "Negative quantity rejected");
    await database.GetCollection<Product>("Products").UpdateOneAsync(x => x.Id == product.Id, Builders<Product>.Update.Set(x => x.Stock, 2).Set(x => x.DiscountPrice, 900.10m));
    cartPage = WebUtility.HtmlDecode(await Get("/Cart"));
    Check(cartPage.Contains("2 adet ürün") && cartPage.Contains("1.800,20"), "Cart reconciles changed stock and prices");
    await database.GetCollection<Product>("Products").UpdateOneAsync(x => x.Id == product.Id, Builders<Product>.Update.Set(x => x.Stock, 5).Set(x => x.DiscountPrice, 1000.25m));
    await Post("/Cart/Remove", new() { ["id"] = product.Id }, "/Cart");
    Check(WebUtility.HtmlDecode(await Get("/Cart")).Contains("Sepetiniz henüz boş"), "Remove product from cart");
    var externalReturn = await Post("/Cart/Add", new() { ["id"] = product.Id, ["quantity"] = "1", ["returnUrl"] = "https://example.test" }, "/");
    Check(externalReturn.Headers.Location!.ToString() == "/Cart", "Cart blocks external return URL");
    await Post("/Cart/Clear", new(), "/Cart");
    Check(WebUtility.HtmlDecode(await Get("/Cart")).Contains("Sepetiniz henüz boş"), "Clear cart");
    await Post("/Cart/Add", new() { ["id"] = product.Id, ["quantity"] = "1" }, "/");
    ...
    Check(WebUtility.HtmlDecode(await Get("/Cart")).Contains("Sepetiniz henüz boş"), "Inactive products removed from cart");
    await Post("/Cart/Add", new() { ["id"] = product.Id, ["quantity"] = "1" }, "/Cart");
    Check(WebUtility.HtmlDecode(await Get("/Cart")).Contains("Sepetiniz henüz boş"), "Inactive product cannot be added");
